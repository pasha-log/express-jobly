...
class Company {
   ...
   * */
	...
	static async findAll(searchParams = {}) {
		// Destructure the search filters
		const { minEmployees, maxEmployees, name } = searchParams;
		...
		if (minEmployees !== undefined) {
			queryItems.push(minEmployees);
			whereStatements.push(`num_employees >= $${queryItems.length}`);
		}

		if (maxEmployees !== undefined) {
			queryItems.push(maxEmployees);
			whereStatements.push(`num_employees <= $${queryItems.length}`);
		}

		if (name) {
			queryItems.push(`%${name}%`);
			whereStatements.push(`name ILIKE $${queryItems.length}`);
		}
		...
		const companiesRes = await db.query(queryStr, queryItems);
